...
import { Search, Download, ArrowUpRight, ChevronLeft, ChevronRight } from "lucide-react";
import { useDatabase } from "../context/DatabaseContext";

export function TransactionsTable() {
  const db = useDatabase();
  const completedTransactions = db.transactions.filter((tx) => tx.status === "Success");
  const totalPages = Math.max(1, Math.ceil(completedTransactions.length / 10));
    ...
    <div className="h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 p-8 border-b border-slate-200 bg-white">
        <div>
          ...
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Completed payment logs only</p>
        </div>
      ...
      <div className="bg-white flex-1 overflow-hidden flex flex-col">
        ...
        <div className="p-10 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-6">
          <p className="text-slate-300 font-bold text-xs uppercase tracking-widest">Page 1 of {totalPages} &bull; {completedTransactions.length} records</p>
